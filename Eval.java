...
public class Eval {
	...
	//
	//Decision theory principle of maximum expected utility in a game-
	//playing context, in a multi-agent environment. 
	//Static function that returns a real-valued evaluation of the 
	//quality of the given state of play. The value is positive if the 
	//state is good for the human user. Function must calculate the 
	//expected utility of state by considering every possible outcome 
	//of dice roll and resulting utility of those possible outcomes.
	//value_rolled_hand function already calculates the expected utility
	//of a game state where the dice have already been rolled.
	static double value_roll_hand(State s, int depth) {
		double val = 0.0; // return value
		//Must analyze all 3 (three) dice.
		//Die 1 (one)... checks validity
		for (DieFace d1 : DieFace.values()) {
			if (d1 != DieFace.invalid) {
				//Die 2 (two)... checks validity
				for (DieFace d2 : DieFace.values()) {
					if (d2 != DieFace.invalid) {
						//Die 3 (three)... checks validity
						for (DieFace d3 : DieFace.values()) {
							if (d3 != DieFace.invalid) {
								//Calculate the expected utility value.
								//Declare variables for probability and 
								//outcome as type "double" (probs, answer)
								double probs, answer;
								probs = s.rollProb(d1, d2, d3);
								s.roll(d1, d2, d3);
								//Call value_rolled_hand function
								answer = value_rolled_hand(s, depth);
								val += (probs * answer);
							}
						}
					}
	...
	//Author: Michael Wallace, 12/08/2018
	//
	//Static function that returns a real-valued evaluation of the quality
	//of given state of play. Function does NOT perform any game-tree search
	//to determine value. It quickly calculates a heuristic estimate of the 
	//quality of current state of play. Function is used to evaluate non-terminal
	//states of maximum depth of game tree. Evaluation value should be positive if 
	//the state is good for the computer player, and negative if the state if good
	//for the human user. Function should also handle any valid state of play. 
	static public double heuristic(State s) {
		// Heuristic value to be returned ...
		double value = 0.0;
		//Computes a heuristic evaluation value for a state of play without performing
		//any look-ahead search. If the comp_brains_eaten function are greater than the 
		//user_brains_eaten function, then set the value to State.win_payoff.
		//Else, set the value to negative State.win_payoff.
		if (s.comp_brains_eaten > s.user_brains_eaten) {
